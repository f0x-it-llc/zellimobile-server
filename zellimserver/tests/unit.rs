//! unit — pure-function unit tests for zellimserver.
//!
...
fn session_name_accepts_valid() {
    for name in &["foo", "my-session_1", "abc", "A", "session-123", "a_b_c"] {
        assert!(
            zellimserver::ipc::validate_session_name(name).is_ok(),
...
fn session_name_rejects_empty() {
    assert!(zellimserver::ipc::validate_session_name("").is_err());
}
...
fn session_name_rejects_path_separator() {
    assert!(zellimserver::ipc::validate_session_name("a/b").is_err());
    assert!(zellimserver::ipc::validate_session_name("../x").is_err());
}
...
fn layout_name_accepts_valid() {
    for name in &["compact", "default", "my-layout", "layout_2"] {
        assert!(
            zellimserver::grpc::helpers::validate_layout_name(name).is_ok(),
...
fn layout_name_rejects_empty() {
    assert!(zellimserver::grpc::helpers::validate_layout_name("").is_err());
}

#[test]
fn layout_name_rejects_abs_path() {
    assert!(zellimserver::grpc::helpers::validate_layout_name("/abs/path").is_err());
}
...
fn layout_name_rejects_slash() {
    assert!(zellimserver::grpc::helpers::validate_layout_name("a/b").is_err());
}

#[test]
fn layout_name_rejects_kdl_extension() {
    // "foo.kdl" contains a "." which is outside [A-Za-z0-9_-].
    assert!(zellimserver::grpc::helpers::validate_layout_name("foo.kdl").is_err());
}
...
fn pane_id_terminal() {
    ...
    let id = zellimserver::actions::pane_id_from_target(42, false);
...
fn pane_id_plugin() {
    use zellij_utils::data::PaneId;
    let id = zellimserver::actions::pane_id_from_target(7, true);
...
mod control_msg {
    ...
    use zellimserver::control::ControlRequest;
...
mod config_precedence {
    ...
    fn default_constant_is_loopback_50051() {
        assert_eq!(zellimserver::config::DEFAULT_BIND, DEFAULT_BIND);
    }
}
...
fn parse_version_standard() {
    // Mirror the parse logic from bin/zellimserver.rs `parse_zellij_version`.
...
fn san_entry_from_env_empty_when_unset() {
    ...
    let result = zellimserver::tls::SanEntry::from_env();
...
fn tls_generate_includes_extra_ip_san() {
    ...
    use std::net::{IpAddr, Ipv4Addr};
    use zellimserver::tls::{SanEntry, generate_self_signed_pem};
...
fn tls_generate_always_includes_localhost() {
    use zellimserver::tls::generate_self_signed_pem;

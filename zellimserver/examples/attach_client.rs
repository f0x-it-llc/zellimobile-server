...
use zellimserver::proto::client_frame::Kind as ClientKind;
use zellimserver::proto::server_frame::Kind as ServerKind;
use zellimserver::proto::zelli_client::ZelliClient;
use zellimserver::proto::{AttachReq, ClientFrame, LoginRequest};
...
fn parse_args() -> Result<Args> {
    ...
    Ok(Args {
        ...
        type_text: get("--type")
            .unwrap_or_else(|| "echo zellimserver_b3_ok > /tmp/b3_proof.txt\n".to_owned())
            .replace("\\n", "\n"),
...
async fn main() -> Result<()> {
    ...
    let mut client = ZelliClient::new(channel.clone());
    ...
    let mut authed_client =
        ZelliClient::with_interceptor(channel, move |mut req: tonic::Request<()>| {
